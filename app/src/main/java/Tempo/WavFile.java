...
public class WavFile {
    ...
    public static WavFile openWavFile(File file, int a) throws IOException, WavFileException {
        BUFFER_SIZE = a;
        ...
        while (true) {
            ...
            if (chunkID == FMT_CHUNK_ID) {
                ...
                if (wavFile.numChannels == 0)
                    throw new WavFileException("Number of channels specified in header is equal " +
                            "to zero");
                if (wavFile.blockAlign == 0)
                    throw new WavFileException("Block Align specified in header is equal" +
                            " to zero");
                if (wavFile.validBits < 2)
                    throw new WavFileException("Valid Bits specified in header is less" +
                            " than 2");
                if (wavFile.validBits > 64)
                    throw new WavFileException("Valid Bits specified in header is greater" +
                            " than 64, this is greater than a long can hold");
